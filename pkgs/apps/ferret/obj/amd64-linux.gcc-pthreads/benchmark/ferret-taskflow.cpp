...
// #include "/home/zhicheng/taskflow/taskflow/algorithm/pipeline.hpp"
#include "/home/zhicheng/taskflow/taskflow/algorithm/data_pipeline.hpp"
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
	...
	tf::Taskflow taskflow("ferret_pipeline");
	tf::Executor executor(TOKEN_IN_FLIGHT);

	
	// create a pipeline graph
	tf::DataPipeline pl(TOKEN_IN_FLIGHT,
		tf::make_data_pipe<void, void*>(tf::PipeType::SERIAL, [&](tf::Pipeflow& pf) {
			void* ret = my_load_filter(nullptr);
			if (ret == NULL) pf.stop();
			return ret;
		}),

		tf::make_data_pipe<void*, void*>(tf::PipeType::PARALLEL, [&](void* item) {
			return my_seg_filter(item);
		}),

		tf::make_data_pipe<void*, void*>(tf::PipeType::PARALLEL, [&](void* item) {
			return my_extract_filter(item);
		}),

		tf::make_data_pipe<void*, void*>(tf::PipeType::PARALLEL, [&](void* item) {
			return my_vec_filter(item);
		}),

		tf::make_data_pipe<void*, void*>(tf::PipeType::PARALLEL, [&](void* item) {
			return my_rank_filter(item);
		}),
	...
	taskflow.composed_of(pl).name("ferret_pipeline");
